...
(define (histogram img) (cond
                          [(bitmap? img) (histoRec (pixs->val (getImgPixs img)) (list))]
                          [(pixmap? img) (map (lambda (data) (list (hex->rgb (first data)) (second data)))
                                              (histoRec (pixs->val (getImgPixs (imgRGB->imgHex img))) (list)))]
                          [(hexmap? img) (histoRec (pixs->val (getImgPixs img)) (list)) ]))
...
(define (compress img) (if (compressed? img)
                           img
                           (subCompress img (maxHistogramData (histogram img)))))
